...
function startBot(api, chats, lists, users, anonymousUsers) {
  ...
  function read(message, username, thread_id, userId, otherUsernames, otherIds, callback) {
    ...
    currentOtherIds = otherIds.filter(function(v) {
      return v !== MARC_ID;
    });
  ...
  function ignore(msg, sendReply) {
    ...
    if (text === "ignore") {
      if (users[currentUserId] && users[currentUserId][currentThreadId] && users[currentUserId][currentThreadId].state === "ignored") {
        return sendReply({
          text: "I'm already ignoring you. If you want me to stop ignoring you please do /unignore."
        });
      }
      users[currentUserId] = users[currentUserId] || {};
      users[currentUserId][currentThreadId].state = "ignored";
      return sendReply({
        text: "Messages from you will now be ignored. Do /unignore to stop being ignored."
      });
    } else {
      if (users[currentUserId] && users[currentUserId][currentThreadId].state === "ignored") {
        delete users[currentUserId][currentThreadId];
        return sendReply({
          text: "Oh hey " + currentUsername + " you're there!"
        });
      }

      return sendReply({
        text: "Sorry I'm not ignoring you. If you want me to, do /ignore."
      });
    }
  }
  ...
  function sendPrivate(msg, sendReply) {
    ...
    if (words.length < 2) return {
      text: 'Usage: /send-private name : message'
    };
  ...
  function reminders(msg, sendReply) {
    ...
    if (!currentChat.hasOwnProperty("timezone")) return sendReply({
      text: "Please set your timezone with the /settimezone command"
    });
    ...
    return sendReply({
      text: "Reminder at: " + date.toISOString().replace(/T/, ' ').replace(/\..+/, '') + " --> '" + rest.replace(ret[0].text, '') + '\''
    });
  }
  ...
  function staticText(msg, sendReply) {
    var possibilities = [
      ...
      [
        ...
        ["I'm a long story... About 24h long.", "I'm not too sure", "I never really asked myself this question."]
      ],
      [
        [/(^\/sayit)/i],
        ["David's an idiot"]
    ...
    for (var i = 0; i < possibilities.length; i++) {
      ...
      for (var j = 0; j < possibleMatches.length; j++) {
        ...
        if (match && match.length > 0) {
          return sendReply({
            text: randFrom(possibilities[i][1])
          });
        }
      }
    }
  ...
  function pickup(msg, sendReply) {
    ...
    if (list.length === 1) return sendReply({
      text: "Hey, " + currentOtherUsernames[~~(currentOtherUsernames.length * Math.random())] + "! ;) " + randFrom(lines)
    });

    var name = list[1];
    if (name === "me") return sendReply({
      text: "No."
    });

    return sendReply({
      text: "Hey, " + capitalize(name) + "! ;) " + randFrom(lines)
    });
  ...
  function sendStickerBigSmall(msg, sendReply) {
    var possibilities = [
      [
        [/( |^)(small|big)( |$)/i],
        [767334526626290, 767334556626287, 767334506626292]
      ]
    ];
    for (var i = 0; i < possibilities.length; i++) {
      var possibleMatches = possibilities[i][0];
      for (var j = 0; j < possibleMatches.length; j++) {
        ...
        if (match && match.length > 0) {
          return sendReply({
            stickerID: randFrom(possibilities[i][1])
          });
        }
      }
    }
  ...
  function slap(msg, sendReply) {
    ...
    var list = arr.split(/\s+/);
    if (list.length === 1) return sendReply({
      text: currentOtherUsernames[~~(currentOtherUsernames.length * Math.random())] + " just got slapped."
    });

    var name = list[1];
    if (name === "me") return sendReply({
      text: currentUsername + " just got slapped." + (Math.random() > 0.5 ? " Hard." : "")
    });

    if (anonymousUsers[name]) {
      api.sendMessage(getAnonymous(currentUserId) + " just slapped you.", anonymousUsers[name]);
      return sendReply({
        text: name + " was told that they got slapped."
      });
    }

    return sendReply({
      text: capitalize(name) + " just got slapped." + (Math.random() > 0.5 ? " Hard." : "")
    });
  }
  ...
  function subtractScore(msg, sendReply) {
    ...
    if (contains(currentOtherUsernames, name)) {
      ...
      return sendReply({
        text: name + "'s score is now " + score + "."
      });
    }
    return sendReply({
      text: "Who's " + name + "?"
  ...
  function spank(msg, sendReply) {
    ...
    var name = list[1];
    if (list.length === 1) return sendReply({
      text: randFrom(currentOtherUsernames) + " just got spanked." + (Math.random() > 0.5 ? " So. Hard." : "")
    });

    if (name === "me") return sendReply({
      text: currentUsername + " just got spanked." + (Math.random() > 0.5 ? " So. Hard." : "")
    });
    if (anonymousUsers[name]) {
      api.sendMessage(getAnonymous(currentUserId) + " just spanked you.", anonymousUsers[name]);
      return sendReply({
        text: name + " was told that they got spanked."
      });
    }
    return sendReply({
      text: capitalize(name) + " just got spanked." + (Math.random() > 0.5 ? " So. Hard." : "")
    });
  }
  ...
  function hug(msg, sendReply) {
    ...
    var list = arr.split(/\s+/);
    if (list.length === 1) return sendReply({
      text: randFrom(currentOtherUsernames) + " just got a " + (Math.random() > 0.5 ? "BIG " : "") + "hug."
    });

    var name = list[1];
    if (name === "me") return sendReply({
      text: currentUsername + " just got a " + (Math.random() > 0.5 ? "BIG " : "") + "hug."
    });

    if (anonymousUsers[name]) {
      api.sendMessage(getAnonymous(currentUserId) + " just hugged you.", anonymousUsers[name]);
      return sendReply({
        text: name + " was given a " + (Math.random() > 0.5 ? "BIG " : "") + "hug."
      });
    }

    return sendReply({
      text: capitalize(name) + " just got a " + (Math.random() > 0.5 ? "BIG " : "") + "hug."
    });
  }
  ...
  function weekendText(msg, sendReply) {
    ...
    var today = new Date();
    return sendReply({
      text: (today.getDay() === 0 || today.getDay() === 6 ? "YES" : "NO")
    });
  }

  function refmt(msg, sendReply) {
    var match = matches(/^\/refmt\s+(.+)/i, msg);
    if (!match) return;
    var data = msg.replace("/refmt ", "");

    var spawn = require('child_process').spawn;
    var child = spawn('./node_modules/reason/refmt_impl.native', ['--parse', 'ml', '--print', 're']);
  ...
  function addScore(msg, sendReply) {
    ...
    if (name === currentUsername) {
      return sendReply({
        text: name + ", you can't upvote yourself -_- "
      });
    }
    if (contains(currentOtherUsernames, name)) {
      var score = (currentChat.scores[name] ? currentChat.scores[name] : 0) + 1;
  ...
  function score(msg, sendReply) {
    ...
    name = capitalize(name);
    if (!contains(currentOtherUsernames, name)) return sendReply({
      text: "who?"
    });

    var pts = currentChat.scores[name] ? currentChat.scores[name] : 0;
    return sendReply({
      text: ("" + name + " has " + pts + " points")
    });
  }
  ...
  function bees(msg, sendReply) {
    if (matches(/\b(bees)\b/i, msg)) {
      return sendReply({
        text: "http://cdn.gifstache.com/2012/7/19/gifstache.com_893_1342731571.gif"
      });
    }
  }
  ...
  function sexxiBatman(msg, sendReply) {
    if (matches(/(wanna make some trouble)/i, msg)) {
      return sendReply({
        text: "http://99gifs.com/-img/514e8830afa96f09940128f8.gif"
      });
    }
  }
    ...
    if (msg.indexOf("giphy") > -1) {
      ...
      request.onload = function() {
        if (request.status >= 200 && request.status < 400) {
          ...
          return sendReply({
            text: "No gif for this search result."
          });
        }
      };
  ...
  function arbitraryLists(msg, sendReply) {
    ...
    if (arr.length === 1) return sendReply({
      text: (Object.keys(currentChat.lists).length > 0 ? "Existing Lists: \n" + Object.keys(currentChat.lists).map(function(v, i) {
        return (i + 1) + " - " + v;
      }).join("\n") : "No existing list.")
    });
    ...
    if (keyword === 'new') {
      if (currentChat.lists[listName]) {
        return sendReply({
          text: "List '" + listName + "' already exists."
        });
      }
      if (listName.length > 0) {
        ...
        return sendReply({
          text: "List '" + listName + "' created."
        });
      }
    } else if (keyword === 'delete') {
      if (!currentChat.lists[listName]) {
        return sendReply({
          text: "No list of name '" + listName + "' exists."
        });
      }

      if (!lists[currentChat.lists[listName].id].content) {
        return sendReply({
          text: "List '" + listName + "' is emtpy."
        });
      }
      ...
      if (arr.length > 3) {
        ...
        if (isNaN(num)) return sendReply({
          text: num + " isn't an an item number in the list " + listName + "."
        });

        if (num - 1 >= lists[currentChat.lists[listName].id].content.length || num - 1 < 0) {
          return sendReply({
            text: "Item " + num + " in list '" + listName + "' doesn't exist."
          });
        }
        ...
        return sendReply({
          text: listName + ": \n" + lists[currentChat.lists[listName].id].content.map(function(v, i) {
            return (i + 1) + " - " + v.data;
          }).join("\n")
        });
      }
      ...
      if (listName.length > 0) {
        ...
        if (currentChat.lists[listName].thread_id !== currentThreadId) return sendReply({
          text: "Sorry you can't delete the list. This list was created in another chat."
        });
      ...
      if (listName.length > 0 && arr.length > 3) {
        ...
        return sendReply({
          text: "Added '" + arr.slice(3).join(' ') + "' to " + listName + "."
        });
      }
      ...
      return sendReply({
        text: "List '" + lists[id].name + "' added to current thread."
      });
      ...
      return sendReply({
        text: "Paste this into another chat to import the list '" + listName + "':\n/list import " + currentChat.lists[listName].id
      });
      ...
      if (arr.length > 3) {
        ...
        return sendReply({
          text: "Item " + num + " was added by " + item.creator
        });
      }

      return sendReply({
        text: "Usage: /list blame list-name item-number"
      });
      ...
      if (!lists[currentChat.lists[keyword].id]) {
        ...
        return sendReply({
          text: "Cannot find list with id `" + id + "`.  Attempting to repair."
        });
      } else if (!lists[currentChat.lists[keyword].id].content) {
        return sendReply({
          text: "List '" + keyword + "' is emtpy."
        });
      }
      return sendReply({
        text: keyword + ": \n" + lists[currentChat.lists[keyword].id].content.map(function(v, i) {
          return (i + 1) + " - " + v.data;
        }).join("\n")
      });
    }
  ...
  function topScore(msg, sendReply) {
    ...
    return sendReply({
      text: "Top Score: " + maxName + ", with " + max + " points."
    });
  }
  ...
  function hashUsername(name) {
    var arr = name.split('').map(function(v) {
